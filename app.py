...
def get_args():
    ...
    t_desc = "The type of model: POSE, TEXT, TEXT_REC or CAR_META"
...
def create_output_image(model_type, image, output):
    ...
    if model_type == "POSE":
    	...
    	for detected_box in output:
    		...
    		confidence = detected_box[2]
    		print(confidence, 'confidence')
    		if confidence > conf_threshold:
	    		x_min = detected_box[3] * image.shape[0]
	    		y_min = detected_box[4] * image.shape[1]
	    		x_max = detected_box[5] * image.shape[0]
	    		y_max = detected_box[6] * image.shape[1]
		    
    	# similar to pose i guess 
    	# Get only face detections above 0.5 confidence, set to 255
        # for c in range(len(output)):
        #     output[c] = np.where(output[c]>0.5, 255, 0)
        #
        # for box in output:
        ...
        # 		xmin = int(box[4] * width)
        # 		ymin = int(box[5] * height)
        return image
    else:
